...
exports.run = (client, message, args) => {
 if(!message.member.hasPermissions("MANAGE_EMOJIS")) return message.channel.send(":no_entry: Bu komutu kullanabilmek için `Emojileri yönet` yetkisine sahip olmalısınız")
    ...
    if (!link) return message.channel.send('Emojinin alınacağı linki girmelisin. <a:hayr:639887269720752128>')
    if (!isim) return message.channel.send('Emojinin ismini belirlemedin <a:hayr:639887269720752128>')
